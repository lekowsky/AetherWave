package pl.skyrise.vendingMachine.command;
...
import pl.skyrise.vendingMachine.gui.EditorGUI;
...
public class VendingCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> SUBCOMMANDS = Arrays.asList(
            ...
            "stock", "restock", "restocklist", "list", "reload", "info", "tp", "help"
    );

    public VendingCommand(VendingMachine plugin) {
        this.plugin = plugin;
    }
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) { sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm create <nazwa>")); return; }
        String name = args[1];
    ...
    private void handleDelete(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) { sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm delete <nazwa>")); return; }
    ...
    private void handleEdit(CommandSender sender, String[] args) {
        if (!(sender instanceof Player p)) { sender.sendMessage("§cTylko gracze!"); return; }
        if (!p.hasPermission("vendingmachine.edit")) { noPermission(p); return; }
        if (args.length < 2) { p.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm edit <nazwa>")); return; }
    ...
    private void handleOpen(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) { p.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm open <nazwa>")); return; }
        MachineTemplate t = plugin.getMachineManager().getTemplate(args[1]);
        if (t == null) { p.sendMessage(plugin.getPrefix() + msg("not-found").replace("{name}", args[1])); return; }
    ...
    /**
     * /vm stock info <szablon>           - info defaultów stocku w szablonie
     * /vm stock fill                     - wypełnia automat na który patrzysz
     */
    private void handleStock(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm stock <info|fill> [szablon]"));
            return;
        }
        ...
        switch (action) {
            case "fill" -> {
                ...
                p.sendMessage(plugin.getPrefix() + ColorUtil.color(
                        "&aWypełniono zapasy automatu &e" + placement.getTemplateName() + "&a do max!"));
            }
            case "info" -> {
                if (args.length < 3) {
                    sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm stock info <szablon>"));
                    return;
                }
                ...
                sender.sendMessage(ColorUtil.color("&6&l--- Stock szablonu: " + t.getName() + " ---"));
                sender.sendMessage(ColorUtil.color("&7Aktywnych automatów: &e" + placements));
                sender.sendMessage(ColorUtil.color("&7Auto-restock: " + (t.isAutoRestockEnabled() ? "&aWł" : "&cWył") +
                        " &7(co " + t.getAutoRestockInterval() + " min, +" + t.getAutoRestockAmount() + " szt.)"));
                sender.sendMessage(ColorUtil.color("&7Itemy (defaulty):"));
                for (VendingItem item : t.getItems().values()) {
                    String stockInfo = item.isUnlimitedStock() ? "&a∞" : "&7max: &e" + item.getMaxStock();
                    sender.sendMessage(ColorUtil.color("  &7" + item.getId() + ": " + stockInfo + " &7- &f" + item.getDisplayName()));
                }
            }
        }
    }
    ...
    /**
     * /vm restock <szablon> - natychmiastowy restock wszystkich automatów danego szablonu
     */
    private void handleRestock(CommandSender sender, String[] args) {
        if (!sender.hasPermission("vendingmachine.admin")) { noPermission(sender); return; }
        if (args.length < 2) { sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm restock <szablon>")); return; }
        MachineTemplate t = plugin.getMachineManager().getTemplate(args[1]);
        ...
        sender.sendMessage(plugin.getPrefix() + ColorUtil.color(
                "&aWykonano restock &e" + count + " &aautomatu/ów typu &e" + t.getName() +
                        " &a(+" + t.getAutoRestockAmount() + " szt./item)"));
    }

    /**
     * /vm restocklist - lista automatów do uzupełnienia
     */
    private void handleRestockList(CommandSender sender) {
        ...
        sender.sendMessage(ColorUtil.color("&6&l========= AUTOMATY DO UZUPELNIENIA ========="));

        if (emptyList.isEmpty() && lowList.isEmpty()) {
            sender.sendMessage(ColorUtil.color("&aWszystkie automaty maja wystarczajaco zapasow! ✓"));
            return;
        }

        if (!emptyList.isEmpty()) {
            sender.sendMessage(ColorUtil.color("&c&l--- WYPRZEDANE (" + emptyList.size() + ") ---"));
        ...
        if (!lowList.isEmpty()) {
            sender.sendMessage(ColorUtil.color("&e&l--- NISKI STOCK (" + lowList.size() + ") ---"));
        ...
        sender.sendMessage(ColorUtil.color("&8Lacznie: &c" + emptyList.size() + " wyprzedanych &7| &e" +
    ...
    private void handleInfo(CommandSender sender, String[] args) {
        if (!sender.hasPermission("vendingmachine.admin")) { noPermission(sender); return; }
        if (args.length < 2) { sender.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm info <nazwa>")); return; }
        MachineTemplate t = plugin.getMachineManager().getTemplate(args[1]);
        if (t == null) { sender.sendMessage(plugin.getPrefix() + msg("not-found").replace("{name}", args[1])); return; }
        int placements = plugin.getPlacementManager().getPlacementCount(t.getName());
        sender.sendMessage(ColorUtil.color("&6&l--- Info: " + t.getName() + " ---"));
        sender.sendMessage(ColorUtil.color("&7Tytuł: &f" + t.getTitle()));
        sender.sendMessage(ColorUtil.color("&7Rzędy: &f" + t.getRows()));
        sender.sendMessage(ColorUtil.color("&7Włączony: " + (t.isEnabled() ? "&aTak" : "&cNie")));
        sender.sendMessage(ColorUtil.color("&7Nexo ID: &f" + (t.getNexoFurnitureId() != null ? t.getNexoFurnitureId() : "Brak")));
        sender.sendMessage(ColorUtil.color("&7Przedmioty: &f" + t.getItems().size()));
        sender.sendMessage(ColorUtil.color("&7Aktywne automaty: &f" + placements));
        sender.sendMessage(ColorUtil.color("&7Auto-restock: " + (t.isAutoRestockEnabled() ? "&aCo " + t.getAutoRestockInterval() + " min (+" + t.getAutoRestockAmount() + ")" : "&cWył")));
    }

    private void handleTeleport(CommandSender sender, String[] args) {
        if (!(sender instanceof Player p)) return;
        if (!p.hasPermission("vendingmachine.admin")) { noPermission(p); return; }
        if (args.length < 2) { p.sendMessage(plugin.getPrefix() + ColorUtil.color("&c/vm tp <nazwa>")); return; }
        ...
        p.sendMessage(plugin.getPrefix() + ColorUtil.color("&aTeleportowano!"));
    }

    private void sendHelp(CommandSender sender, String label) {
        sender.sendMessage(ColorUtil.color(msg("help-header")));
        sender.sendMessage(ColorUtil.color("&e/" + label + " create <nazwa> &7- Utwórz szablon"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " delete <nazwa> &7- Usuń szablon"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " edit <nazwa> &7- Edytuj szablon"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " open <nazwa> &7- Otwórz podgląd"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " remove &7- Usuń automat (patrz na blok)"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " stock info <szablon> &7- Info o defaultach"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " stock fill &7- Wypełnij automat (patrz na blok)"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " restock <szablon> &7- Restock wszystkich automatów"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " restocklist &7- Lista do uzupełnienia"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " info <nazwa> &7- Informacje"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " list &7- Lista szablonów"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " tp <nazwa> &7- Teleport do automatu"));
        sender.sendMessage(ColorUtil.color("&e/" + label + " reload &7- Przeładuj"));
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) {
            ...
            switch (sub) {
                case "delete", "edit", "open", "info", "tp", "restock" -> {
    ...
    private boolean hasPermissionForSub(CommandSender sender, String sub) {
        return switch (sub) {
            ...
            case "edit" -> sender.hasPermission("vendingmachine.edit");
