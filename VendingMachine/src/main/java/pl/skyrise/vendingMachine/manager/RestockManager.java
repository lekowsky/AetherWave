...
public class RestockManager {
    ...
    public RestockManager(VendingMachine plugin) {
        this.plugin = plugin;
            ...
            if (restocked > 0) {
                ...
                if (plugin.getConfig().getBoolean("auto-restock.broadcast-restock", false)) {
                    String msg = ColorUtil.color(plugin.getConfig().getString(
                            ...
                            "&7[&6Automat&7] &aUzupelniono zapasy: &e{template}"
    ...
    private void startLowStockScan() {
        ...
        lowStockScanTask = Bukkit.getScheduler().runTaskTimer(plugin, this::scanAllForLowStock, ticks, ticks);
    ...
    public void scanAllForLowStock() {
        ...
        String summary = plugin.getPrefix() + ColorUtil.color(
                "&e⚠ Automaty wymagaja uzupelnienia! &cWyprzedane: &f" + emptyCount +
